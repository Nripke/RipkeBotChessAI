...
public class MyBotUnrestricted : IChessBot
{
    /*
        RIPKEBOT v 1.4
    ...
    */
    ...
    int search(int depth, int alpha, int beta)
    {
        ...
        if (board.IsInCheckmate()) {return board.IsWhiteToMove ? negInf : inf;}
        if (board.IsDraw()) {return 0;}
        if (depth == 0) {return horizonSearch(3, negInf, inf);}
        ...
        if (board.IsWhiteToMove)
        {
            ...
            foreach (Move m in moves)
            {
                ...
                if (s > bestEval) {bestEval = s; bestMove[maxDepth-depth] = m;} //killerMoves[maxDepth-depth].Add(m);
                if (bestEval >= beta) {return beta;}
                alpha = max(alpha, bestEval);
            }
        }else {
            ...
            {
                int s = move(m, depth, alpha, beta, true);
                if (s < bestEval) {bestEval = s; bestMove[maxDepth-depth] = m;} 
                if (bestEval < alpha) {return alpha;}
                beta = min(beta, bestEval);
            }
        }
    ...
    Move[] pruneOrdering (Boolean captures, int depth)
    {
        ...
        for (int i=0; i<moves.Length; i++)
        {
            ...
            if (m.Equals(bestMove[maxDepth-depth])) {vals[i] += 100000;}
    ...
    int evaluation()
    {
        ...
        for (int i=0; i<12; i++)
        {
            foreach (Piece p in pl[i])
            {
                ...
                eval += (pieceVals[pType] + getPositionalValue(pType, p.IsWhite, p.Square.File, p.Square.Rank) + 10*BitboardHelper.GetNumberOfSetBits(BitboardHelper.GetPieceAttacks(p.PieceType, p.Square, board, p.IsWhite)))*color;
                //
                //System.Console.WriteLine(positVal);
            }
        }
